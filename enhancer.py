...
if __name__ == '__main__':
    ...
    # 2. Input Path (Pick one of your new low-res images)
    #    Let's find the first image in the 'test_images_lr' folder
    lr_folder = 'test_images_lr'
    try:
        first_lr_image = os.listdir(lr_folder)[0]
        input_file = os.path.join(lr_folder, first_lr_image)
    ...
    output_file = os.path.join(output_folder, 'result_' + first_lr_image)

    # --- Run the Enhancement ---
    if not os.path.exists(model_file):
        print(f"Error: Model file not found at '{model_file}'.")
    else:
        enhance_image(model_path=model_file, input_path=input_file, output_path=output_file)